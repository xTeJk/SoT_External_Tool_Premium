...
{
	char SET_NameBuf[48]; 
	sprintf(SET_NameBuf, "SoT External Tool Premium");
...
void cUIRenderer::DrawMenu(IDirect3DDevice9* pDevice)
{
	...
	ImGui::Begin("SEA OF THIEVES EXTERNAL TOOL PREMIUM", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse);
	...
	else if (SETTINGS::MENU::activeMenuTab == 1) //-> TREASURES TAB
	{
		...
		else if (SETTINGS::MENU::activeMenuTab_treasures == 1)
		{
			ImGui::BeginChild("SOT_EXTERNAL_TOOL_TREASURES_TAB_1_1", ImVec2(168, 400), true);
			UIHandler->UIChildHeaderSingle("COMMON ITEMS LIST", true, 158);
			for (int i = 0; i < _countof(SETTINGS::TOOLS::TREASURES_ESP::GENERAL::treasuresCommon); ++i)
			{
				bool buttonState = false;
				(SETTINGS::TOOLS::TREASURES_ESP::GENERAL::treasuresCommonActiveTab) == i ? buttonState = true : buttonState = false;
				UIHandler->drawButton(SETTINGS::TOOLS::TREASURES_ESP::GENERAL::treasuresCommon[i], i, "TREASURES_TAB_1_1", buttonState, 140, 30);
			}

			ImGui::EndChild();
			ImGui::SameLine();

			ImGui::BeginChild("SOT_EXTERNAL_TOOL_TREASURES_TAB_1_2", ImVec2(648, 400), true);
			UIHandler->UIChildHeaderSingle("GENERAL SETTINGS", true, 648);

			TreasuresManager->treasuresUI(TREASURE_TYPE_COMMON, SETTINGS::TOOLS::TREASURES_ESP::GENERAL::treasuresCommonActiveTab);

			ImGui::EndChild();
		}
		else if (SETTINGS::MENU::activeMenuTab_treasures == 2)
		{
			ImGui::BeginChild("SOT_EXTERNAL_TOOL_TREASURES_TAB_2", ImVec2(835, 400), true);

			UIHandler->UIChildHeaderSingle(SETTINGS::MENU::menuTabs_treasures[SETTINGS::MENU::activeMenuTab_treasures], false, 835);

			ImGui::EndChild();
		}
		
		else if (SETTINGS::MENU::activeMenuTab_treasures == 3)
		{
			ImGui::BeginChild("SOT_EXTERNAL_TOOL_TREASURES_TAB_3", ImVec2(835, 400), true);
		...
		{
			ImGui::BeginChild("SOT_EXTERNAL_TOOL_TREASURES_TAB_x", ImVec2(835, 400), true);
	...
	else if (SETTINGS::MENU::activeMenuTab == 5) //-> UTILITY TAB
	{
		...
		UIHandler->UIChildHeaderSingle("x", true, 413);
		ImGui::EndChild();
	}
